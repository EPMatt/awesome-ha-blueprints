...
<Input
  ...
  description='Delay used for debouncing RAW controller events, when the device is connected with an integration which needs debounce. Increase this value only if the device is integrated with deCONZ, ZHA and you notice duplicate action runs.'
  selector='number'
/>
