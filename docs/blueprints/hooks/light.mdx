...
<Input
  ...
  description='Time (in milliseconds) the light should take turn on or off, if the light supports it.'
  selector='number'
/>
<Input
  name='Step Duration'
  description='Time duration (in milliseconds) for each step, both when controlling brightness and color. Tweaking it might result in a smoother control experience, depending on the light model.'
  selector='number'
/>
<Input
